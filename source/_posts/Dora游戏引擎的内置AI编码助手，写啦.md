...
&emsp;&emsp;先看看目前实现的效果，在 Android 手机上安装的引擎运行起来是什么样吧。

![dora-agent-mobile](Dora游戏引擎的内置AI编码助手，写啦/dora-agent-mobile.jpg)
...
&emsp;&emsp;相比 nanobot 的原始设计，Dora 在压缩切分时稍微增加了一个改进：对话边界保护。

&emsp;&emsp;简单说，就是尽可能找到一段"对话起始到结束"的安全边界来切分历史窗口，而不是从任意位置一刀切。这样被压缩的历史片段能保留尽可能完整的上下文，LLM 生成摘要时能理解"这段对话在干什么"，而不是从半截对话开始压缩，导致信息丢失。

&emsp;&emsp;具体实现上，我们会向前或向后搜索，找到用户消息边界（`role === 'user'`）作为切分点，确保每次压缩的历史片段都是完整的多轮对话。
